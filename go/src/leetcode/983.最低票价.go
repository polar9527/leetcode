...
func mincostTickets(days []int, costs []int) int {
	...
	dp = func(cur int) int {
		...
		expenses[cur] = math.MaxInt32
		next := cur
		for k := 0; k < len(costs); k++ {
			for next < len(days) && days[next] < days[cur]+durations[k] {
				next++
			}
			expenses[cur] = min(expenses[cur], dp(next)+costs[k])
		}
		return expenses[cur]
	}
	return dp(0)
